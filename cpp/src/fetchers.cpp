...
std::string carries_userinfo(const std::string& locator) {
    return "download: " + locator +
           " carries userinfo in its authority, and the three transports this layer speaks for "
           "end userinfo at three different bytes";
}

// RFC 3986 section 3.2's delimiters and nothing else, for the schemes no socket
// on this platform opens: file and smb are read off a filesystem by path_of, and
// a scheme with no fetcher is refused by name before a host is ever consulted.
std::string offline_authority(const std::string& locator, std::string& authority) {
    const auto at = locator.find("://");
    if (at == std::string::npos && locator.rfind("//", 0) != 0) {
        return "";
    }
    std::string rest = at == std::string::npos ? locator.substr(2) : locator.substr(at + 3);
    rest = rest.substr(0, rest.find_first_of("/?#"));
    if (rest.find('@') != std::string::npos) {
        return carries_userinfo(locator);
    }
    if (!rest.empty() && rest.front() == '[') {
        const auto close = rest.find(']');
        if (close == std::string::npos) {
            return "download: " + locator + " has an unclosed IPv6 literal";
        }
        authority = rest.substr(1, close - 1);
        return "";
    }
    const auto colon = rest.rfind(':');
    authority = colon == std::string::npos ? rest : rest.substr(0, colon);
    return "";
}

bool over_the_network(const std::string& locator) {
    std::string scheme = locator.substr(0, locator.find("://"));
    for (char& c : scheme) {
...
std::string transport_authority(const std::string& locator, std::string& authority) {
    authority.clear();
    if (!over_the_network(locator)) {
        return offline_authority(locator, authority);
    }
#ifdef _WIN32
    const std::wstring url = wide(locator);
    URL_COMPONENTS parts{};
    parts.dwStructSize = sizeof(parts);
    parts.dwSchemeLength = parts.dwHostNameLength = parts.dwUserNameLength =
        parts.dwPasswordLength = static_cast<DWORD>(-1);
    if (!::WinHttpCrackUrl(url.c_str(), static_cast<DWORD>(url.size()), 0, &parts)) {
        return "download: " + locator + " is not a URL the transport reads";
    }
    if (parts.dwHostNameLength == 0) {
        return "download: " + locator + " names no host";
    }
    // Not dwUserNameLength: WinHttpCrackUrl reports a zero-length user name for
    // `https://@hf.co/x`, where net/url and urllib both see userinfo and refuse.
    const auto after = locator.find("://") + 3;
    if (locator.substr(after, locator.find_first_of("/?#", after) - after).find('@') !=
        std::string::npos) {
        return carries_userinfo(locator);
    }
    authority = narrow(std::wstring(parts.lpszHostName, parts.dwHostNameLength));
    if (authority.size() >= 2 && authority.front() == '[' && authority.back() == ']') {
        authority = authority.substr(1, authority.size() - 2);
    }
    return "";
#else
    // https_available() is false here, so no socket opens to what this reads and
    // the answer is held to the other two implementations by the shared corpus
    // rather than by a transport. The day a fetcher lands on this platform, its
