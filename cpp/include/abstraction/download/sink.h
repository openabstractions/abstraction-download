...
// A path reduced for the LEXICAL containment check below: cleaned of "." and
// "..", forward slashes, no trailing separator, and folded with std::tolower
// per byte when the host is Windows.
//
// That fold is a guess about the host, not a reading of the volume, and it is
// wrong: measured, folding one byte at a time left "café" and "CAFÉ" apart while
// NTFS holds them as one file, and disagreed with what Go and Python did to the
// same input. It stays only
// because containment is a question about a store root's own subtree and both
// sides are folded alike. Whether two paths name one file is NOT answered here:
// that is same_path in <abstraction/download/paths.h>, which asks the volume.
//
