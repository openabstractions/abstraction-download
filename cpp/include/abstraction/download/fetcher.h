...
// The authority of a locator, read by the parser the transport on this platform
// will use -- WinHttpCrackUrl on Windows, the same call that produces the string
// handed to WinHttpConnect. Returns "" and sets `authority`, or the reason this
// machine will not read it.
//
// It lives beside the transport and not beside the policy on purpose. A host
// authorised by one parse and connected to by another is a confused deputy
// however carefully the two parsers were written, so there is one parse.
