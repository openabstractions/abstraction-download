...
// abstraction::download alone declares what is below; abstraction_download_runner
// is what defines it. Reaching this header without linking that target used to
// compile and fail only at link, naming no header at all.
#ifndef ABSTRACTION_DOWNLOAD_RUNNER_LINKED
#error "abstraction/download/runner.h needs abstraction::download_runner linked, not abstraction::download alone"
#endif
...
// trailing dot. "" when the locator names none -- a local path reaches nothing
// -- and "" for an authority this machine will not read.
std::string host_of(const std::string& locator);

// The same host, or the reason this machine cannot tell where the locator would
// connect. Anything deciding where a secret or a connection goes asks this one:
// "" from host_of cannot distinguish a local path from an authority the
// transport and this layer would read differently.
