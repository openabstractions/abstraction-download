...
void test_an_adopted_job_honours_a_pause_before_it_fetches() {
    ...
    // Zero, because held() is `now < expires_at`: a lease of zero has lapsed at
    // every instant, and any positive ttl makes whether the successor may claim
    // depend on how long the lines between here and there took.
    store.claim(job.id, "predecessor", std::chrono::milliseconds(0));
...
// Mirrors TestHostOf and TestAnAuthorityTheTransportReadsDifferentlyIsRefused
// in Go and HostAuthorityTests in Python. The corpus behind these rows was run
// through net/url, urllib.parse and WinHttpCrackUrl at once; every locator in
// the second list is one where at least two of the three disagreed, and on this
// platform WinHttpCrackUrl answered `evil.com` for the first of them while
// host_of answered `hf.co`.
void test_the_authority_is_read_by_the_transports_own_parser() {
    const std::pair<std::string, std::string> plain[] = {
    ...
    for (const auto& [locator, want] : plain) {
        std::string host;
        const std::string why = abstraction::download::vouch_host(locator, host);
        check("a plain authority reads the same everywhere: " + locator,
              why.empty() && host == want);
    }
    ...
    for (const std::string& locator : hostile) {
        std::string host;
        const std::string why = abstraction::download::vouch_host(locator, host);
        check("an authority the transport reads differently is refused: " + locator,
              !why.empty() && host.empty());
        check("and host_of says nothing rather than guessing: " + locator,
              abstraction::download::host_of(locator).empty());
    }
}
...
int main() {
    ...
    test_the_authority_is_read_by_the_transports_own_parser();
