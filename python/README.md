...
**Ready.** Tagged `go/v0.2.1`, cross-language conformance passes, and the
example below runs as shown.
...
that keeps transferring after the requesting program exits (BITS); Linux and
macOS have neither it nor each other's answer, and none of the three is reachable
through one call an application can make on all of them. This layer is that call.
...
Current release [`go/v0.2.1`](https://github.com/openabstractions/abstraction-download/releases/tag/go%2Fv0.2.1).
...
renewal and the final rename stay in the `Runner` either way, which is what lets
one implementation finish what another started.
...
index: `pip install ./cas/python ./watch/python ./job/python ./download/python`
from a checkout of each.
...
`abstraction::download` alone is the header-only reader: the record rules
without the runner. `add_subdirectory(cpp)` gives the same two names, so
vendoring and installing are interchangeable at the call site.
