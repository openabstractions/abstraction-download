...
# The pairs our three implementations were measured disagreeing about, on three
# filesystems that also disagree with each other. Written as escapes because two
# of these pairs are indistinguishable on screen, and that is the point of them.
#
...
class SamePathTests(unittest.TestCase):
    def merges(self, d, a, b):
        """What the volume holding d does with the two names: make one, look for
        the other."""
        made = os.path.join(d, a)
        try:
            fd = os.open(made, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600)
        except OSError as e:
            self.skipTest("this volume will not hold %r: %s" % (a, e))
        os.close(fd)
        try:
            mine = os.stat(made)
            try:
                other = os.stat(os.path.join(d, b))
            except OSError:
                return False
            return os.path.samestat(mine, other)
        finally:
            os.remove(made)

    def test_same_path_agrees_with_the_volume(self):
        """The whole rule. A destination that does not exist yet is the normal
        case -- that is what a download is -- so the pair is asked about before
        either file is created."""
        for name, a, b in SPELLINGS:
            with self.subTest(name), tempfile.TemporaryDirectory() as d:
                got = dl.same_path(os.path.join(d, a), os.path.join(d, b))
                self.assertEqual(got, self.merges(d, a, b), name)

    def test_same_path_agrees_when_the_file_is_there(self):
        for name, a, b in SPELLINGS:
            with self.subTest(name), tempfile.TemporaryDirectory() as d:
                want = self.merges(d, a, b)
                made = os.path.join(d, a)
                fd = os.open(made, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600)
                os.close(fd)
                self.assertEqual(dl.same_path(made, os.path.join(d, b)), want, name)

    def test_same_path_through_a_missing_directory(self):
        with tempfile.TemporaryDirectory() as d:
            want = self.merges(d, "Models", "models")
            got = dl.same_path(
                os.path.join(d, "Models", "x.gguf"), os.path.join(d, "models", "x.gguf")
            )
            self.assertEqual(got, want)

    def test_asking_leaves_nothing_behind(self):
        with tempfile.TemporaryDirectory() as d:
            dl.same_path(os.path.join(d, "Kelvin.gguf"), os.path.join(d, "kelvin.gguf"))
            self.assertEqual(os.listdir(d), [])

    def test_obvious_cases(self):
        with tempfile.TemporaryDirectory() as d:
            p = os.path.join(d, "a.gguf")
            self.assertTrue(dl.same_path(p, p))
            self.assertFalse(dl.same_path("", p))
            self.assertTrue(dl.same_path("", ""))
            self.assertTrue(
                dl.same_path(os.path.join(d, "a", "..", "b.gguf"), os.path.join(d, "b.gguf"))
            )
...
class HostAuthorityTests(unittest.TestCase):
    """Mirrors TestHostOf and its siblings in reach_test.go. The corpus behind
    these was run through net/url, urllib.parse and WinHttpCrackUrl at once, and
    the rows below are where the three disagreed."""

    def test_a_plain_authority_reads_the_same_everywhere(self):
        for locator, want in {
            ...
            B + B + "nas" + B + "share" + B + "x.bin": "nas",
            "//nas/share/x.bin": "nas",
            "C:" + B + "models" + B + "x.bin": "",
            "/mnt/models/x.bin": "",
            "gopher://example.invalid/x": "example.invalid",
        }.items():
            self.assertEqual(dl.host_of(locator), want, locator)

    def test_an_authority_the_transport_reads_differently_is_refused(self):
        for locator in [
            "https://hf.co" + B + "@evil.com/x",
            "https://evil.com" + B + "@hf.co/x",
            "https://hf.co" + B + ".evil.com/x",
            "https://hf.co" + chr(9) + ".evil.com/x",
            "https://hf.co" + chr(10) + ".evil.com/x",
            "https://hf.co" + chr(13) + ".evil.com/x",
            "https://hf.co%40evil.com/x",
            "https://hf.co%2f.evil.com/x",
            "http://user:pw@cdn.example:8443/a",
            "https://user@evil.com@hf.co/x",
            "https://hf.co:443:8080/x",
            "https://ｈｕｇｇｉｎｇｆａｃｅ.ｃｏ/x",
            "https://huggingface。co/x",
            "https://☃.example/x",
        ]:
            with self.assertRaises(dl.Unreachable, msg=locator):
                dl.vouched_host(locator)
            self.assertEqual(dl.host_of(locator), "", locator)

    def test_a_credential_is_not_attached_to_an_authority_we_cannot_read(self):
        os.environ["ABSTRACTION_CRED_HF"] = "secret"
        os.environ["ABSTRACTION_CRED_HF_HOSTS"] = "hf.co"
        try:
            src = dl.Source(
                scheme="https",
                locator="https://hf.co" + B + "@evil.com/model.bin",
                attrs={dl.CREDENTIAL_ATTR: "hf"},
            )
            with self.assertRaises(dl.Unreachable):
                dl.headers_for(src)
        finally:
            del os.environ["ABSTRACTION_CRED_HF"]
