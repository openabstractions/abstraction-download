...
func proven(store job.Store, digest, except string) []Source {
	...
	for _, rec := range all {
		...
		if err != nil || st.IsDir() || size <= 0 || st.Size() != size || held(seen, st) {
			continue
		}
		seen = append(seen, st)
...
// held reports whether this file is already among the sources. Every candidate
// has been stat'd by the time it gets here, so the volume can be asked outright
// and there is no spelling to reduce. SamePath is for the harder case where the
// file is not there yet.
func held(seen []os.FileInfo, fi os.FileInfo) bool {
	for _, s := range seen {
		if os.SameFile(s, fi) {
			return true
		}
