...
func (s *client) ResumeOrSubmit(spec Spec, requires ...string) (Handle, Outcome, error) {
	...
	live, done := s.recordsFor(spec.Sink)
	if live != nil {
...
func (s *client) recordsFor(sink Sink) (live, complete *job.Record) {
	_, final, _ := LocalSink(s.runner.Store, "", sink)
	// Unreadable ids are dropped for want of anywhere to put them, exactly as in
	// client.inFlight: a corrupt record for this destination reads as no record,
	// and the caller starts over. One corrupt record used to hide all of them.
	...
	for _, rec := range all {
		...
		_, got, _ := LocalSink(s.runner.Store, "", spec.Sink)
		if !SamePath(got, final) {
			continue
		}
		switch {
...
// canonicalPath is the string a destination is KEYED on, which is a different
// question from whether two paths name one file — SamePath answers that, and it
// answers it by asking the volume. A key cannot: destinationID hashes this
// string to pick a record id before any record exists, so it must be derivable
// from the path alone, and two spellings a case-folding volume will merge can
// still hash apart. The scan in recordsFor runs first and uses SamePath, so the
// key is a race window rather than the decision.
//
