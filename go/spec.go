...
type Spec struct {
	...
	// Request identifies this handoff to a delegate, and is the same string on
	// every attempt at the same job.
	//
	// It is not stored and no caller sets it: Runner.Delegate fills it in from
	// the record's own id at the moment the work is offered. That is what makes
	// it stable across a reconnect — the delegate can be asked what it did with
	// this request by a process that never made it, after a reboot, in another
	// language. See Locator.
	//
...
//
// A RELATIVE path here is relative to the store root, on whichever machine
// resolves it. Never to a working directory: the process that writes the bytes
// may have a different one, a different user, or be on another machine, and a
// record is read by all three. An ABSOLUTE path names one machine's filesystem
// and is delivered by that machine — see PortableSink for who declines it and
// why. There is no third reading, and the two are not interchangeable.
//
// Client.Get is where a person's two strings become one of these, and it is the
// exception that proves the rule: its destination is a shell path, so it is
// resolved against the caller's working directory before a Sink exists at all.
type Sink struct {
...
// comparablePath reduces a path for the LEXICAL containment check above:
// cleaned, forward slashes, no trailing separator, and folded with strings.
// ToLower when the host is Windows.
//
...
// root's own subtree and both sides are folded alike. Whether two paths name one
// file is NOT answered here: that is SamePath, which asks the volume.
