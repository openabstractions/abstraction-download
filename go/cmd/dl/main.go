...
// parseGet reads the whole command line or refuses it.
//
// A flag dl does not honour is a refusal, never a skip. Skipping it tells a
// caller who typed --verify instead of --digest that an unverified file was
// checked, which is the one thing this program exists to be trusted about.
func parseGet(args []string) (url, dest, digest string, err error) {
	url = args[0]
	...
	fs.Usage = func() {}
	into := fs.String("o", ".", "directory or file to download into")
	want := fs.String("digest", "", "sha256:<hex>, the artifact's identity")
	if err := fs.Parse(args[1:]); err != nil {
		return "", "", "", err
	}
	if fs.NArg() > 0 {
		return "", "", "", fmt.Errorf("does not know what to do with %q", fs.Arg(0))
	}
	// An empty value is the same lie as an unknown flag: `--digest "$D"` with D
	// unset would download and verify nothing, and say it succeeded.
	var blank string
	...
	if blank != "" {
		return "", "", "", fmt.Errorf("-%s wants a value", blank)
	}
	return url, *into, *want, nil
}

func cmdGet(ctx context.Context, args []string) {
	url, out, digest, err := parseGet(args)
	switch {
	case errors.Is(err, flag.ErrHelp):
		usage()
		return
	case errors.Is(err, errNotAURL):
		usage()
		os.Exit(2)
	case err != nil:
		fmt.Fprintln(os.Stderr, "dl:", err)
