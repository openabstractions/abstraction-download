...
// A record this reader is too old to decode is left alone, not allowed to
// refuse the sweep: ErrUnreadable travels beside the records that did decode
// (job.ErrUnreadable says why), and returning it alone would let one forged
// record hide every other job from a supervisor.
func (p *replay) orphans() string {
	rs, err := p.store.Orphans()
	var unreadable *job.ErrUnreadable
