...
// Assurance is how a capability claim is established here, and it is the only
// thing that says what a false one costs.
//
// Capabilities() reports what an implementation SAYS it can do. While we wrote
// every implementation, saying and doing were the same thing and nothing needed
// to tell them apart. They are not the same thing for a program somebody else
// wrote, and the difference has already been paid for once: a delegate claiming
// CapResume and starting from zero discarded 6,586,368 proven bytes with the
// digest still matching and nothing red (Runner.Delegate).
type Assurance string

const (
	// Checked: the core does the work again, so a false claim costs nothing.
	Checked Assurance = "checked"
	// Falsifiable: nothing here proves the claim true, but one observation
	// refutes it, and a refuted claim is disbelieved (Disbelieve).
	Falsifiable Assurance = "falsifiable"
	// Recovered: nothing refutes it and nothing needs to, because a false claim
	// costs work that is taken back and done again.
	Recovered Assurance = "recovered"
	// Trusted: nothing here refutes a false claim and nothing recovers from it.
	// The list is short on purpose and it is published, because a promise
	// nobody checks is one an adopter is entitled to see named.
	Trusted Assurance = "trusted"
)

// capabilities is the roster, and a word that is not in it is not a capability.
//
// The grade sits against the constant so that adding one without saying what a
...
//
// Unknown is ignored rather than refused, which is the opposite of what
// Declaration.Valid does with a term it cannot read, and the asymmetry is the
// point: a capability only ever GRANTS, so a word we cannot read grants
// nothing and a newer plugin talking to an older core simply gets less. A
// constraint we cannot read would hand a delegate exactly the jobs its author
// wrote the constraint to exclude.
func Assured(c Capability) (Assurance, bool) {
	a, ok := capabilities[c]
	return a, ok
}

// AllCapabilities is the roster in name order, for anything that documents or
// reports it.
...
// FailureExtension is where a record carries the class of its last failure,
// alongside the sentence in Record.Error.
//
// Record.Error is prose and a sentence is not a class. An error rebuilt from it
// alone is a bare string, so Permanent on it is false however the attempt
// ended, including for the refusals this layer declares forever — and the two
// endings are the whole retry model, unobservable in the one place a caller can
// branch on them.
//
// It is an extension rather than a field because the record is three
...
func setFailure(rr *job.Record, err error) error {
	if err == nil {
		clearFailure(rr)
		return nil
	}
...
// sentence goes would answer "is this over" about an attempt nobody can read.
...
// intact, and returns nil for a record that has not failed.
//
...
// noFetcherFor says which half of For's question failed.
//
// For asks two things at once — does anything serve this scheme, and does
// anything serving it promise these capabilities — and one answer for both
// blames a condition that was met. An application met `scheme "http"` four
// times while the HTTP fetcher was registered and had just served three
// downloads.
func noFetcherFor(fs *Fetchers, src Source, requires []string) error {
	if _, served := fs.For(src, nil); served {
		return fmt.Errorf("%w: nothing serving scheme %q promises %s",
			ErrNoFetcher, src.Scheme, strings.Join(requires, " and "))
	}
	return fmt.Errorf("%w: scheme %q", ErrNoFetcher, src.Scheme)
}
