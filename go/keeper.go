...
func (k *keeper) hold() {
	...
	t := time.NewTicker(renewEvery(k.ttl))
	defer t.Stop()
	for {
		select {
		case <-k.done:
...
// renewEvery is how often a lease of this length is extended: three renewals
// per TTL, so a single missed one — a share that blinks, a record being
// replaced at the moment we read it — is not the end of the lease. The cost of
// each is one small file write, and every path that holds a lease pays it at
// this rate whether it is driven by a timer or by bytes landing.
...
// fencing reports whether a refused store write means this owner no longer
// holds the work, as against a write that failed for its own reasons.
func fencing(err error) bool {
	return errors.Is(err, job.ErrStaleEpoch) || errors.Is(err, job.ErrLeaseExpiry)
}
