...
//
// The lease is expired on demand rather than waited out. A recall with no grace
// moves ExpiresAt to now, which is the store's own way of ending a lease early
// and the only one that does not ask how fast this machine writes: the TTL below
// is never reached, so no number here asserts a speed. Claim replaces the whole
// lease, so the recall marker does not survive into whoever takes over next.
func stageDeadOwner(t *testing.T, store job.Store, id string, done, verified int64) {
	t.Helper()
	held, err := store.Claim(id, "dead-owner", time.Hour)
	if err != nil {
	...
	if _, err := store.Recall(id, held.Lease.Epoch, "the owner is gone", "test", 0); err != nil {
		t.Fatalf("stage lapse: %v", err)
	}
	rec, err := store.Load(id)
	if err != nil {
		t.Fatal(err)
	...
	if rec.Lease.Held(time.Now()) {
		t.Fatalf("staging did not take: %s still holds the lease", rec.Lease.Owner)
	}
...
type leaseWatchingStore struct {
	job.Store
	ttl time.Duration
}

func (s *leaseWatchingStore) Claim(id, owner string, ttl time.Duration) (*job.Record, error) {
	s.ttl = ttl
	return s.Store.Claim(id, owner, ttl)
}

// Staging a crashed owner must end its lease, never sit through it. The
// comparison is the helper's own TTL against the helper's own elapsed time, so
// it says nothing about how fast this machine is: a helper that waits out its
// lease fails here on every machine, and one that expires it passes on every
// machine.
func TestStagingEndsItsLeaseRatherThanWaitingForIt(t *testing.T) {
	_, store, root := newRunner(t)
	body, digest := payload(t, 4<<10)
	id := submit(t, store, root, digest, int64(len(body)),
		Source{Scheme: "https", Locator: rangeServer(t, body).URL})
	watched := &leaseWatchingStore{Store: store}

	start := time.Now()
	stageDeadOwner(t, watched, id, 2<<10, 1<<10)
	if waited := time.Since(start); waited >= watched.ttl {
		t.Fatalf("staging spent %v of a %v lease: the lease was waited out, not ended", waited, watched.ttl)
	}
}
