...
// Close: a share is a path. There is nothing to release.
...
// Locate finds a request this delegate accepted, for a Start whose answer was
// lost on the way back.
//
// The window is real and it is the wide kind: Submit writes a record across a
// share, the supervisor on the other side may claim it within milliseconds, and
// an SMB write that lands and then reports an error is an ordinary Tuesday. Sent
// and refused look identical from here, and without this the facade falls back
// and the same artifact is fetched twice.
//
// The request is carried in the remote record's own spec, so nothing new is
// written to the share and no index has to be kept true. It reads every record
// rather than one, which is the price of not inventing a second name for a job
// the store already names; this runs only when a handoff is in doubt.
func (d *Delegator) Locate(ctx context.Context, request string) (string, error) {
	if request == "" {
		return "", errors.New("nas: a handoff with no request identity cannot be found again")
	}
	if err := d.Available(); err != nil {
		return "", err
	}
...
func (d *Delegator) CanServe(spec download.Spec) bool {
	...
	// A destination only the submitter's machine has. This delegate would resolve
	// `C:\models\x.gguf` against its own filesystem and write somewhere useless,
	// or nowhere, while the application waits for a file that was never coming.
	// The submitter's own runner delivers it instead, which is the case the
	// refusal in Runner leaves open on purpose.
	if !download.PortableSink(spec.Sink) {
		return false
	}
	for _, src := range spec.Sources {
		if download.ThisMachine(src.Locator) {
			return false
		}
	}
	return true
