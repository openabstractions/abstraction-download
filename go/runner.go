...
type Runner struct {
	...
	// NotServing are systems this process was told never to use, whatever the
	// machine says — the operator's flag, which survives a Rebind because a
	// person switching a tier on in a window is answering a different question
	// from an operator running one supervisor deliberately a tier lower.
	NotServing []string

	// bound is the machine's answer the chain above was built from, so Rebind
	// can tell it has nothing to do without re-probing a share to find out.
	bound string
...
func (r *Runner) Run(ctx context.Context, id string) error {
	...
	if err := r.run(ctx, rec, epoch); err != nil {
		...
		letGo := r.release(id, epoch)
		if wrote != nil || letGo != nil {
			return errors.Join(err, wrote, letGo)
		}
		return err
	}
	...
	return r.release(rec.ID, epoch)
}

// release hands the lease back, and takes a terminal record as already released.
//
// Arriving at CANCELLED or FAILED is itself the letting go: nothing may claim a
// finished job, so there is no holding left to give up, and the store refuses
// every write from that epoch including the one that would say so. That refusal
// is the job layer's declared contract in three languages, so the caller either
// duplicates its table of terminal states or asks and believes the answer —
// and a copy of somebody else's state table is a copy that drifts.
//
// Every other refusal is real. Discarding them is how four days of cancelled
// downloads sat holding a lease nobody was using and nothing said a word.
func (r *Runner) release(id string, epoch int64) error {
	if err := r.Store.Release(id, epoch); err != nil && !errors.Is(err, job.ErrTerminal) {
		return err
	}
	return nil
}
...
func (r *Runner) run(ctx context.Context, rec *job.Record, epoch int64) error {
	...
	if want := spec.Artifact.Digest; want != "" {
		...
		if !sameDigest(got, want) {
			...
			// runner resumes onto a prefix already known to be wrong — and if
			// either half is refused the record still claims bytes that are
			// gone, which is the same resume by a worse route, so both refusals
			// go back with the mismatch.
			swept := os.Remove(partial)
			_, cleared := r.Store.Update(rec.ID, epoch, func(rr *job.Record) error {
				rr.Progress.Done = 0
...
// wrote to.
//
// DL-R3 forbids deleting a partial to make a record true, and this does not
// break it: a file of no bytes proves no prefix, so nothing is being taken
// away except something the next run would have to explain.
func removeEmptyPartial(partial string) {
...
func (r *Runner) honour(want job.Want, id string, epoch int64) error {
	...
	case job.WantCancel:
		...
		return err
	case job.WantPause:
		// Nothing to write. Paused means nobody is working on it, and a held
		// lease tells every reader the opposite — including the status line a
		// person is watching to see whether their pause took effect — but the
		// lease is given back by Run's one exit, and releasing here as well made
		// that second release fail with ErrLeaseExpiry against a lease this
		// owner had already handed back.
		//
		// Letting go is safe precisely because Orphans excludes paused jobs, so
		// it does not invite the next sweep to start the job again. Those two
...
func (r *Runner) refuseUnportableSink(s Sink) error {
	if !r.SharedStore || PortableSink(s) {
		return nil
	}
	p := s.Final
	if PortableSink(Sink{Final: p}) {
		p = s.Partial
	}
	return fmt.Errorf("%w: %s", ErrUnportableSink, p)
}

// PortableSink reports whether a sink names a place every machine sharing the
// store can find — a path under the store root — rather than one machine's
// filesystem.
//
// Asked by whoever is about to move the work across a machine boundary: a
// supervisor over a shared store before it adopts, a delegator before it hands
// the job to a box that mounts the store somewhere else. An absolute sink is not
// wrong; it is unanswerable by anyone but the machine that named it.
func PortableSink(s Sink) bool {
...
func (r *Runner) fetch(ctx context.Context, rec *job.Record, spec Spec, epoch int64, f *os.File, h hash.Hash, rolling bool, gap fetchRange, proven *Ranges, seen *Validators, onIntent func(job.Want)) (int64, bool, error) {
	...
	// this owner's first chance to hear that the lease it claimed is already
	// somebody else's.
	...
	// A refused renewal means this owner no longer holds the job, and the next
	// one may already be writing to the same file. Nothing below can return an
	// error from inside Fetch's progress callback, so the refusal is kept here
	// and the transfer's own context is cancelled to stop the bytes — the same
	// mechanism the parallel path uses. See TestARefusedRenewalStopsEveryRange.
	var lost error
	fetchCtx, dropped := context.WithCancel(ctx)
	defer dropped()
	...
	for _, src := range sources {
		...
		// Ahead of every other reading of err, because err here is the
		// cancellation this owner asked for and reporting that would hide why.
		if lost != nil {
			return proven.VerifiedPrefix(), restarted, lost
		}
		if err == nil {
...
// readable splits what a sweep answered from what it could not read.
//
// job.List and job.Orphans return *job.ErrUnreadable ALONGSIDE their records,
// never instead of them, so a store holding one corrupt file is a store a sweep
// must still work with. Treating that as a failed call denies service to every
// job that decoded because of one that did not; dropping it lets a half-blind
// sweep report itself healthy. So the records come back and the refusal travels
// beside them, for the caller to carry out with its own answer.
func readable(recs []*job.Record, err error) (kept []*job.Record, unread, fatal error) {
	var u *job.ErrUnreadable
...
func (r *Runner) Adopt(ctx context.Context) (int, error) {
	orphans, unread, err := readable(r.Store.Orphans())
	if err != nil {
		return 0, err
	}
	n := 0
	...
	for _, o := range orphans {
		...
		// Neither is a job whose destination only the submitting machine has.
		// Claiming it in order to refuse it spends that machine's attempt, bumps
		// the backoff, and writes this supervisor's policy into a record the
		// submitter's own client reads back as the caller's error — so a
		// destination that worked before this supervisor existed stops working
		// on the machine that can still deliver it. Run still refuses out loud
		// when somebody asks for this job by name.
...
// SharedStoreRoot reports whether a store root is written by machines other than
// this one, from the only evidence a program has without being told: a UNC root
// names somebody else's machine.
//
// Not a complete answer, and it does not try to be. A mapped drive letter and an
// NFS mount point are shares that look local, and a supervisor over one of those
// is told so — see ABSTRACTION_SHARED_STORE in cmd/jobd. What this closes is the
// case where the evidence is in the path itself.
//
// Both separators are read on every platform, deliberately, and filepath is the
// wrong package here: a store root is data that travels, so a supervisor on
// Linux reads roots written by Windows. filepath.ToSlash is the identity
// function off Windows, which made `\\nas\share` local there and shared here —
// one root, two machines, two answers.
func SharedStoreRoot(root string) bool {
	return len(root) >= 2 && pathSeparator(root[0]) && pathSeparator(root[1])
}
